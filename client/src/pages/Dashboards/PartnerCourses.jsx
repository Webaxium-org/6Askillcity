                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      <span className="px-3 py-1 rounded-full bg-secondary text-secondary-foreground text-[10px] font-bold uppercase tracking-wider">
                        {course.type}
                      </span>
                  ...
                  {course.program?.eligibilityChecklist && course.program.eligibilityChecklist.length > 0 ? (
                    <div className="mb-4 flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground bg-muted/30 px-2.5 py-1.5 rounded-lg border border-border/50 w-fit">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/75 animate-pulse" />
                      <span>Requires {course.program.eligibilityChecklist.length} Documents</span>
                    </div>
                  ) : (
                    <div className="mb-4" />
                  )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded-xl border border-border bg-muted/20">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider block">University Partner</span>
                        <span className="text-sm font-bold text-foreground">{activeDetailCourse.program?.university?.name}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl border border-border bg-muted/20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider block">Course Type</span>
                          <span className="text-sm font-bold text-foreground capitalize">
                            {activeDetailCourse.program?.programType === "skill"
                              ? "Skill / Certificate"
                              : "Degree / PG"}
                          </span>
                        </div>
                    ...
                    <div className="p-4 rounded-xl border border-border bg-muted/20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider block">Duration</span>
                          <span className="text-sm font-bold text-foreground">{activeDetailCourse.duration}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 rounded-xl border border-border bg-muted/30">
                    ...
                    <div className="space-y-2.5">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-muted-foreground">Registration / Application Fee</span>
                        <span className="font-bold text-foreground">₹{activeDetailCourse.fee?.applicationFee?.toLocaleString() || "0"}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-muted-foreground">Tuition / Program Fee</span>
                        <span className="font-bold text-foreground">₹{activeDetailCourse.fee?.tuitionFee?.toLocaleString() || "0"}</span>
                      </div>
                      <div className="h-px bg-border my-1" />
                      <div className="flex justify-between items-center">
                        <span className="font-black text-xs uppercase text-primary">Total Course Fee</span>
                        <span className="text-lg font-black text-primary">₹{activeDetailCourse.fee?.totalFee?.toLocaleString() || "0"}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {activeDetailCourse.program?.eligibilityChecklist && activeDetailCourse.program.eligibilityChecklist.length > 0 ? (
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                        {activeDetailCourse.program.eligibilityChecklist.map((item, idx) => (
                          <div
                            key={idx}
