  ...
  const getChartData = () => {
    ...
    if (reportId === "financial-report") {
      return [
        ...
        { name: "Team", value: data.splits.toTeam },
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest">Fee Structure</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-center">Students</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-right">Expected (₹)</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-right">Collected (₹)</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-right">Pending (₹)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50">
              {data.map((item, idx) => (
                <tr key={idx} className="hover:bg-muted/10 transition-colors">
                  <td className="px-6 py-4 font-bold">{item._id || "No Fee Assigned"}</td>
                  <td className="px-6 py-4 font-black text-center">{item.count}</td>
                  <td className="px-6 py-4 font-black text-right">₹{item.totalExpected?.toLocaleString()}</td>
                  <td className="px-6 py-4 font-black text-right text-emerald-600">₹{item.totalPaid?.toLocaleString()}</td>
                  <td className="px-6 py-4 font-black text-right text-rose-600">₹{(item.totalExpected - item.totalPaid).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      return (
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest">Entity</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-right">Amount (₹)</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-right">Share (%)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50">
              {Object.entries(data.splits).map(([key, value]) => (
                <tr key={key} className="hover:bg-muted/10 transition-colors">
                  <td className="px-6 py-4 font-bold capitalize">{key.replace("to", "")}</td>
                  <td className="px-6 py-4 font-black text-right">₹{value.toLocaleString()}</td>
              ...
              <tr className="bg-primary/5">
                ...
                <td className="px-6 py-4 font-black text-right text-primary">₹{data.total.toLocaleString()}</td>
                <td className="px-6 py-4 font-black text-right text-primary">100%</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest">Category / Period</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-center">Count</th>
              <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-right">Percentage</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/50">
            {data.map((item, idx) => (
        ...
        <div className="bg-card border border-border rounded-[2.5rem] overflow-hidden shadow-sm">
          ...
          <div className="p-6 border-b border-border bg-muted/10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
             <div className="flex flex-wrap items-center gap-4">
                ...
                {(reportId === "admission-report" || reportId === "center-admission") && (
                  <div className="flex items-center gap-2">
          ...
          <div className="p-8">
            {loading ? (
              <div className="py-32 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-sm font-bold text-muted-foreground animate-pulse">Compiling Report Data...</p>
              </div>
            ) : view === "chart" ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="p-6 bg-card border border-border rounded-[2rem] shadow-sm">
              ...
              <p className="text-3xl font-black">
                {reportId === "financial-report" ? "₹" + data.total.toLocaleString() : data.reduce((acc, curr) => acc + curr.count, 0)}
              </p>
            </div>
            <div className="p-6 bg-card border border-border rounded-[2rem] shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Report Category</p>
              <p className="text-xl font-black text-primary capitalize">{reportId.split("-")[0]}</p>
            </div>
          </div>
        )}
      </div>
